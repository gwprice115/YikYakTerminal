...
/**
 * For a file of Yaks, prints features in SVMlight format.
 * 
...
public class FeatureSelector {
	...
	/**
	 * For training: reads in Yaks, creates an external dictionary file, and
	 * then outputs feature file w/ labels in the same file.
	 * 
	 * @param inputFiles
	 *            - files of Yaks
	 * @param filenameaddon
	...
	/**
	 * For testing: reads in Yaks, reads in external dictionary file, and then
	 * outputs feature file w/ labels in different file.
	 * 
	 * @param testFiles
	 *            - files of Yaks
	 * @param dictionaryFile
	 *            - file containing words to their feature number
	 * @param filenameaddon
	...
	public FeatureSelector(List<String> testFiles, String dictionaryFile,
			String filenameaddon, boolean[] featureFlags) {
		vocabulary = new HashSet<String>();
		dictionaryMap = new HashMap<String, Integer>();
		fileExt = filenameaddon;
		this.featureFlags = featureFlags;
	...
	/**
	 * FirstPass is only called on training. It iterates through all Yaks in all
	 * files and stores all unigrams and bigrams in a dictionaryMap of String to
	 * feature index. It then writes this out to a file.
	 * 
	 * @param inputFiles
	 *            - files of Yaks
			...
			for (String inputFile : inputFiles) {
				...
				while ((sentence = reader.readLine()) != null) {
					if (sentence.matches("_+")) {
						...
						// this method reads the next line, which should be the
						// Yak,
						...
						reader.close();
						throw new RuntimeException(
								"Number of lines of a yikyak isn't what was expected");
					}
				}
				reader.close();
			}
	...
	/**
	 * for a single yak, appropriately parses and stores all unigrams
	 * @param yakText - content of a Yak
	 */
	...
	/**
	 * secondPass is called in both training or testing, with a boolean
	 ...
	 * 
	 * @param inputFiles- files of Yaks
	...
	private void secondPass(List<String> inputFiles, boolean train) {
		try {
			...
			PrintWriter featureW = new PrintWriter(new FileWriter(featureFile
					+ ".txt")); // features of cases
			PrintWriter labelW = new PrintWriter(new FileWriter(fileExt
					+ ".labels.txt")); // labels of cases
			// get vocab & store yaks
			Pattern headerPattern = Pattern.compile(".*###\\s(.*)\\s###"); // group(1)==header
			Pattern likeLinePattern = Pattern
					.compile("\\s*(-?\\d+)\\slikes.*Posted\\s+([0123456789-]+)\\s+([0123456789:]+).*");
			// group(1)==number of likes. group(2)==date posted. group(3)==time
			// posted.
			...
			for (String inputFile : inputFiles) {
				...
				if (m.matches()) {
					...
					throw new RuntimeException(
							"Problems with school/hour matching in filename");
				}
				reader = new BufferedReader(new FileReader(inputFile));
				while ((sentence = reader.readLine()) != null) {
					if (sentence.matches("_+")) {
						// this is a new yikyak.
						reader.readLine(); // number
						maybeHeader = reader.readLine(); // date collected &
															// possibly header
						try {
							Matcher mHeader = headerPattern
									.matcher(maybeHeader);
							mHeader.matches();
						...
						// bigrams
						// HashMap<String,Integer> bigramBOW = new
						// HashMap<String,Integer>();
						// for (int i= 0; i < yakWords.size()-1; i++){
						// String bigram =
						// yakWords.get(i)+" "+yakWords.get(i+1);
						// int count = bigramBOW.get(bigram)==null ? 0:
						// bigramBOW.get(bigram);
						...
						try {
							...
							calendar.setTime(new SimpleDateFormat(
									"yyyy-MM-dd HH:mm:ss").parse(mLike.group(2)
									+ " " + mLike.group(3)));
							//date is 1-7, subtracting 1 to make 0-6
							dayOfWeek = calendar.get(Calendar.DAY_OF_WEEK) - 1;
							postTime = calendar.get(Calendar.SECOND)
									+ calendar.get(Calendar.MINUTE) * 60
									+ calendar.get(Calendar.HOUR_OF_DAY) * 3600;
						} catch (Exception e) {
							reader.close();
							throw new RuntimeException(
									"Problem with parsing likes: " + e);
						}
						...
						try {
							...
							reader.close();
							throw new RuntimeException(
									"Problem with parsing comments: " + e);
						}
					} else {
						reader.close();
						throw new RuntimeException(
					...
					if (train) {
						...
						if (fileExt.equals(FeatureSelector.ALL_SCHOOLS)) {
							if (likes <= 2) {
								featureW.print(0 + " ");
								labelW.println("-1"); // prints likes only in
														// separate file for
														// testing
								printingThisFeature = true;
							} else if (likes > 23) {
					...
					if (printingThisFeature) {
						...
						if (featureFlags[0]) {
							while (!gramFeatureMap.isEmpty()) {
								Entry<Integer, Integer> feature = gramFeatureMap
										.pollFirstEntry();
						...
						if (featureFlags[1]) {
							// dayOfWeek (each is a separate feature that
							// receives a 1 if it is that day.)
							featureW.print(nextIndex + dayOfWeek + ":1 ");
							nextIndex += 7; // increment next index over all
											// possible day indices
						}
						...
						if (featureFlags[3]) {
							// num capital letters
							featureW.print(nextIndex + ":"
									+ numCapitalLetters(yakText) + " ");
							nextIndex++;
						}
						// if(featureFlags[4]){
						// //length (characters)
	 ...
	 * @param yak - content of a Yak
	 * @return - number of capital letters in the Yak
	 */
	...
	/**
	 * enum for school use as feature
	 */
	public enum School {
	...
	/**
	 * Prints feature files for Claremont and all_schools given which features to include
	 * @param args- which features to include. 1 if include feature.
	 *  CURRENTLY: unigrams, dayOfWeek, header?, numCapitals
	 */
	public static void main(String[] args) {
		...
		schools.add("claremont"); //only do individual for claremont
		...
		for (String school : schools) {
			...
			new FeatureSelector(inputFiles, school,featureFlags);

			List<String> timeTestFiles = new ArrayList<String>();
			timeTestFiles.add(school + "File.time");
			new FeatureSelector(timeTestFiles,"dictionary.txt", school + ".time", featureFlags);

			List<String> randTestFiles = new ArrayList<String>();
			randTestFiles.add(school + "File.rand");

			new FeatureSelector(randTestFiles,"dictionary.txt", school + ".rand", featureFlags);
		}
		...
		new FeatureSelector(allSchoolsTrain,FeatureSelector.ALL_SCHOOLS, featureFlags);
		...
		new FeatureSelector(timeAll,"dictionary.txt", "all_schools.time", featureFlags);
		...
		new FeatureSelector(randAll,"dictionary.txt", "all_schools.rand", featureFlags);
